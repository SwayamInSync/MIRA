...
def _uniform_spherical_sample(
    ...
    vec = np.array([0, 0, 0])
    angle = 2*np.pi*current_pos/num_renders
    radius = np.random.uniform(radius_min, radius_max, 1)
    ...
    correct = False
    while not correct:
        vec[2] = np.random.uniform(-1, 1, 1).tolist()[0]*radius
        if maxz > vec[2] > minz:
            correct = True
    return vec
...
def randomize_camera(
    ...
    x, y, z = _sample_spherical(
        current_pos=current_pos, num_renders=num_renders, radius_min=radius_min, radius_max=radius_max, maxz=maxz, minz=minz
    )
    # x, y, z = _uniform_spherical_sample(
    #     current_pos=current_pos, num_renders=num_renders,radius_min=radius_min, radius_max=radius_max, maxz=maxz, minz=minz
    # )
...
def render_object(
    ...
    Returns:
        None
    """
    output_dir = f"{output_dir}/{os.path.basename(object_file).split('.')[0]}"
    ...
    # randomize and save the lighting parameters
    lights = randomize_lighting()

    lighting_metadata = {
        'key_light': {
            'type': lights['key_light'].data.type,
            'rotation': list(lights['key_light'].rotation_euler),
            'energy': lights['key_light'].data.energy
        },
        'fill_light': {
            'type': lights['fill_light'].data.type,
            'rotation': list(lights['fill_light'].rotation_euler),
            'energy': lights['fill_light'].data.energy
        },
        'rim_light': {
            'type': lights['rim_light'].data.type,
            'rotation': list(lights['rim_light'].rotation_euler),
            'energy': lights['rim_light'].data.energy
        },
        'bottom_light': {
            'type': lights['bottom_light'].data.type,
            'rotation': list(lights['bottom_light'].rotation_euler),
            'energy': lights['bottom_light'].data.energy
        },
    }
    metadata['lights'] = lighting_metadata
